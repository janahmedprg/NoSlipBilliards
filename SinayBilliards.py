...
def arcpart(rad, positions,epsilon):
    xyPhilist = [[],[],[]]
    for ycirc in np.linspace(0,rad,positions):
...
################################################################################
############################### Interactive ####################################
N=400 #number of collisions for each orbit, including passing through the lens wall
startAngPoints=1
xypoints = 100
spin=0
ETAList = [0.8]#[0.1,0.3,0.5,0.7,0.9]
Rlist = [0.2]#[0.1,0.3,0.5,0.7,0.9]
isTor = True
################################################################################
################################################################################
colorMax = xypoints * startAngPoints
epsilon = 0.01
...
for RAD in Rlist:
    for ETA in ETAList:
        ...
        table=make_sinay(RAD,7,isTor)
        xyPhilist = arcpart(RAD,xypoints,epsilon)
        pointsarr = [[],[],[]]
        fname = 'sinay'+'('+str(round(RAD,2))+','+str(round(eta,3))+')'+str(colorMax*N)

        for startang in np.linspace(0,0,startAngPoints):
        ...
        plt.scatter(pointsarr[0],pointsarr[1],s=3,c=pointsarr[2],cmap='gist_rainbow')
        plt.axis('off')
