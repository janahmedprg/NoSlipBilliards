...
def getangle(A):
    # return the angle of the vector A relative to the pos x-axis
    if round(A[0],15)>=0: # was using round 15 for square
        if A[1]>=0:
            return (arcsin((A[1]-eps)/veclen(A)))
        if A[1]<0:
            return (arcsin((A[1]+eps)/veclen(A)))
...
N=11 #number of collisions for each orbit, including passing through the lens wall
XYPoints = 10
ANGPoints = 10
ETAList = list(np.linspace(0.01,0.98,5))
SIDES = 7
StartAng = 7/8*pi #(np.pi+0.0001)/2
EndAng = 7/4*pi #(np.pi*3-0.0001)/2
SPINList = [0,0.3,0.5,0.7,0.9]
        ...
        for x0,y0 in zip(xylist[0],xylist[1]):
            for startangle in np.linspace(StartAng,EndAng,ANGPoints):
                ...
                for i in range(0,N-1):
                    ...
                    wall_com+=(states[i+1,9],)

                wall_set.add(wall_com)
    print(SPIN)
...
# [1,2,0],[1,0,1,0],[1,2,1,0],[1,2,0,1,2,0],[1,0,2,1,2,0],[1,0,1,0,1,0]
...
################################################
################################################
wall_list = wall_list = list(wall_set)
print(len(wall_list))
ETAStart = 0.5
ETAEnd = 0.95
ETAPoints = 500
...
################################################
################################################
angleL = math.radians(AngleL)
angleR = math.radians(AngleR)
etaVSdis = [[],[],[]]
color = 1
lbls =[]
eta_check = []
...
#             #                             continue
#                     for s in range(0,1):
#                         wall_list.append([i,j,k,l,h,s])#,l,h,g,t,ab,ac,s])
    ...
    for eta in np.linspace(ETAStart,ETAEnd,ETAPoints):
        ...
        for i in range(0,len(wall_list[combin])):
            if wall_list[combin][i] == 0:
                ...
                mat_list.append(rotate(angleL+pi))
                mat_list.append(Tr)
                mat_list.append(rotate(-angleL-pi))
            elif wall_list[combin][i] == 2:
                mat_list.append(rotate(-angleR+pi))
                mat_list.append(Tr)
                mat_list.append(rotate(angleR-pi))
...
# plt.savefig('P_3_ES_range'+str(ETAStart)+'-'+str(ETAEnd)+'_iter'+str(ETAPoints)+'_AngleL'+str(AngleL)+'_AngleR'+str(AngleR)+'.png',bbox_inches = "tight")
plt.show()
print('done')
